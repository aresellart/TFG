...
VAE_MODEL_PATH = os.path.join(ROOT, "vanilla_vae_stamps.pt")
RESNET_PATH    = os.path.join(ROOT, "models", "rvl-resnet50.model")
CORRECT_JSON   = os.path.join(ROOT, "correct_images.json")
STAMP_PATH     = "/home/asellart/tfg_stamps/stamp_dataset_grayscale"
...
for p in vae.parameters():
    p.requires_grad = False
print(f"VAE loaded from {VAE_MODEL_PATH} and frozen")
...
def attack_one_class(target_class):
    print(f"\n{'='*55}")
    print(f"Target: {CLASSES[target_class]} (idx {target_class})")
    ...
    out_dir  = os.path.join(ROOT, "attack_results_gpu_vanilla")
    ...
    torchvision.utils.save_image(
        ...
        os.path.join(out_dir,
                     f"stamp_class{target_class}_{cls_name}.png")
    )
    print(f"Saved z and stamp for '{cls_name}'")
    return best_z
...
if __name__ == "__main__":
    ...
    print("\nAll done! Results saved in attack_results_gpu_vanilla/")